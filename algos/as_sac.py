...
if __name__ == "__main__":
    ...
    log_dir = f"runs/profiler/{run_name}"
    ...
    for global_step in range(args.total_timesteps):
        ...
        if (global_step + 1) % eval_every == 0:
            ...
            violation_depth_profiler = ViolationDepthProfiler(gamma=args.gamma)
            for e in range(eval_nb):
                ...
                while not eval_done:
                    ...
                    eval_obs, _, cost, eval_terminated, eval_truncated, eval_infos = eval_envs.step(taus.numpy().clip(-1, 1))
                    costs.append(cost)
                ...
                violation_depth_profiler.add_episode(np.array(costs))
            ...
            writer.dump_csv()
            with open(f"{log_dir}/profiler_step_{global_step}.pkl", "wb") as file:
                pickle.dump(violation_depth_profiler, file)
