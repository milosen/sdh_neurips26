...
class Args:
    ...
    wandb_project_name: str = "cmpo"
    """the wandb's project name"""
    wandb_entity: str = "al-cmpo"
    """the entity (team) of wandb's project"""
    capture_video: bool = False
    """whether to capture videos of the agent performances (check out `videos` folder)"""
    headless_server: bool = False
    """whether the server to capture videos doesn't have a monitor"""
    ...
    kp: float = 0.1
    """K_P, the proportional gain of the PID controller"""
    ki: float = 0.01
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(f"runs/lag/{run_name}")
    writer.add_text(
        "hyperparameters",
    ...
    cost_critic_optimizer = torch.optim.Adam(qcf.parameters(), lr=args.policy_q_lr)
    dual_vars = [log_eta, log_alpha_mean, log_alpha_stddev, log_penalty_temperature]
    ...
    for global_step in range(args.total_timesteps):
        ...
        #cost = infos["cost"] if "cost" in infos else np.array([info["cost"] for info in infos["final_info"]])
        #cost -= args.cost_limit
        ...
        if step_since_last_done >= args.n_step - 1:
            ...
            n_step_discounted_cost = (n_step_cost_rolling_buffer * n_step_gammas).sum()
            rb.add(
            ...
            n_step_discounted_cost = (
                n_step_cost_rolling_buffer[args.n_step - 1 - step_since_last_done :]
                * n_step_gammas[: step_since_last_done + 1]
            ).sum()
            rb.add(
            ...
            for info in infos["final_info"]:
                ...
                if step_since_last_done >= args.n_step - 1:
                    ...
                    for i in range(1, args.n_step):
                        ...
                        n_step_discounted_cost = (n_step_cost_rolling_buffer[i:] * n_step_gammas[:-i]).sum()
                        rb.add(
                    ...
                    for i in range(0, step_since_last_done):
                        n_step_discounted_reward = (n_step_reward_rolling_buffer[i:] * n_step_gammas[:-i]).sum()
                        n_step_discounted_cost = (n_step_reward_rolling_buffer[i:] * n_step_gammas[:-i]).sum()
                        rb.add(
                            n_step_obs_rolling_buffer[i],
                            real_next_obs,
                            n_step_action_rolling_buffer[i],
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % 4 == 0:
                ...
                with torch.no_grad():
                    ...
                    target_qcvalue = (
                        (1 - data.dones.flatten()) * (args.gamma ** (data.bootstrapped_discounts.squeeze(-1))) * target_qcvalue
                    )
                ...
                impr_distr = F.softmax((online_q_values_sampl_actions - lambda_c*online_qc_values_sampl_actions)/ eta.detach(), dim=0)  # shape (N,B)

                # Compute eta and lambda loss: optimization of the normalization, KL regularized, and cost constraints
                q_logsumexp = torch.logsumexp((online_q_values_sampl_actions - lambda_c*online_qc_values_sampl_actions) / eta, dim=0)  # (B,)
                ...
                if sgd_steps % 25 == 0:
                    ...
                    writer.add_scalar("pid/lambda", lambda_c, global_step)
                    writer.add_scalar("pid/derivative", pid.derivative, global_step)
                    writer.add_scalar("pid/integral", pid.integral, global_step)
                    writer.add_scalar("pid/error", pid.error, global_step)
